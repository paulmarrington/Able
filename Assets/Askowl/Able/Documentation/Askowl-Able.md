...
1. It is intanciated with `Stack<T>.Instance`.
2. It is an `IDisposable`. Call `Dispose()` when you are done and the instance will be cached for later - reducing garbage collection.
...
5. The constructor is protected so inherited classes can bypass the caching. Useful if they cache everthing themselves.
...
Cool, huh? But how does it work? Here is the implementation in ***Map.cs***.
...
We could have just used `int index;`. In this case that would probably OK, but in many similar instances we will have problems when one iteration occurs in another. Besides, this is easier to read.
...
* ***`IsNumber`*** returns true of the string is a valid integer or floating point. `Failed` is set also for post-conversion checking.
* ***`Long`*** will return a long for both a whole number string and a floating point representation. It will set Failed if the leaf value was not a number.
* ***`Double`*** will return a double for both a whole number string and a floating point representation. It will set Failed if the leaf value was not a number.
* ***`Boolean`*** will return true of the word starts with ***t*** for true.
...
Sooner or later you will want to serialise or deserialise from or to a tree. For that purpose `Key` will return a string that represents the path from the root to the current location. From there you can iterate through one level of child branches with `FirstChild` and `NextChild`.
...
If you need to know how how many branches to prepare storage, use `Children`, which is an array of the same objects returned above. So, `Children.Length` is the number of child branches.
